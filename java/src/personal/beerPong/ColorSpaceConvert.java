...
/***************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************
 * Utilities for manipulating the type of BufferedImages, and basic color-space conversion.
 **************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************/
public class ColorSpaceConvert
{
    ...
    public static BufferedImage debayerRGGB(String format, int width, int height, byte d[])
    {
        ...
        // loop over each 2x2 bayer block and compute the pixel values for each
        // element.
    ...
    public static BufferedImage convertToImage(String format, int width, int height, byte d[])
    {
        ...
        System.out.println("ColorSpaceConvert: Unknown type " + format);
        return null;
    }
     ...
     * 
     * @param in
     *            The input image, in any format.
     * @param type
     *            The desired type, e.g. BufferedImage.TYPE_3BYTE_BGR
     * @return An image with analagous content as in, but of the requested type. Or, if the input image was already in the requested format, the input image is returned.
     */
    public static BufferedImage convertImage(BufferedImage in, int type)
    {
        if (in.getType() == type)
            return in;
        System.out.println("ColorSpaceConvert: Performing slow image type conversion");
        int w = in.getWidth();
        int h = in.getHeight();
    ...
    {
        return HSVtoRGB(_in, 0xffffff);
    }
    
    public static BufferedImage HSVtoRGB(BufferedImage _in, int outputmask)
    {
        int width = _in.getWidth(), height = _in.getHeight();
        _in = convertImage(_in, BufferedImage.TYPE_INT_RGB);
        int in[] = ((DataBufferInt) (_in.getRaster().getDataBuffer())).getData();
        ...
        for (int y = 0; y < height; y++)
        {
            for (int x = 0; x < width; x++)
            {
                int hsv = in[y * width + x];
                double h = (360 / 255) * ((hsv >> 16) & 0xff);
                double s = ((hsv >> 8) & 0xff) / 255.;
                double v = ((hsv) & 0xff) / 255.;
                if (h >= 360)
                {
                ...
                out[y * width + x] = (((int) (255 * r) << 16) + ((int) (255 * g) << 8) + (int) (255 * b)) & outputmask;
            }
        }
        return _out;
    }
    ...
    public static BufferedImage RGBtoYUV(BufferedImage _in)
    {
        ...
        for (int y = 0; y < height; y++)
        {
            for (int x = 0; x < width; x++)
            {
                ...
                int b = (rgb) & 0xff;
                /*
                 * int yc = clamp(0.299*r + 0.587*g + 0.114*b + 128); int uc = clamp(-0.14713*r - 0.28886*g + 0.436*b + 128); int vc = clamp(0.615*r - 0.51499*g - 0.10001*b + 128);
                 */
                /*
                 * int yc = clamp(((66*r + 129*g + 25*b + 128) >> 8) + 16); int uc = clamp(((-38*r - 74*g + 112*b + 128) >> 8) + 128); int vc = clamp(((112*r - 94*g - 18*b + 128) >> 8) + 128);
                 */
    ...
    public static int[] HSVtoRGB(int hi, int sa, int va)
    {
        double h= hi*(360/255.);
        double s= sa/255.;
        double v= va/255.;
    ...
    public static int[] RGBtoHSV(int r, int g, int b)
    {
        ...
        if (max == 0)
            ...
            s = (int) (255 * (1.0 - ((double) min / (double) max)));
        v = max;
        h = h * 255/360;
